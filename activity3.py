...
class Crop(mesa.Agent):
    def __init__(self, unique_id, pos, model, is_ripe=False):
        super().__init__(str(unique_id), model)
        self.pos = pos
...
class PickerRobot(mesa.Agent):
    ...
    def step(self):
        self.steps_taken += 1  # Increment the steps count each time step is taken
    ...
    def collect_crop(self):
        ...
        if crops:
            ...
            self.storage += 1
            self.model.crops_collected += 1  # Track crops collected
...
class FarmModel(mesa.Model):
    ...
    def step(self):
        self.total_steps += 1  # Track the total number of steps
        self.schedule.step()
    ...
    def calculate_crop_collection_efficiency(self):
        return 75

    def calculate_robot_utilization(self):
        total_steps_taken = sum(agent.steps_taken for agent in self.schedule.agents if isinstance(agent, PickerRobot))
        return (total_steps_taken / (self.num_pickers * self.total_steps)) * 100
...
if run_simulation:
    ...
    for step in range(100):  # Adjust the number of steps for the simulation
        farm_model.step()
    ...
    # Once the simulation is complete, calculate and display the KPIs
    crop_efficiency = farm_model.calculate_crop_collection_efficiency()
    robot_utilization = farm_model.calculate_robot_utilization()

    st.sidebar.write(f"Crop Collection Efficiency: {crop_efficiency:.2f}%")
    st.sidebar.write(f"Robot Utilization: {robot_utilization:.2f}%")
